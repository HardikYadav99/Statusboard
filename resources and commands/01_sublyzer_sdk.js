// search for integration_code in this file and replace with your code
...
(function () {
  const SublyzerSDK = (function () {
      ...
      this.config = Object.assign(
        {
          ...
          apiUrl:
            cfg.apiUrl || "https://sublyzer-backend-production.up.railway.app",
    ...
    SublyzerSDK.prototype.initialize = async function () {
      if (
        ...
        this.config.integrationCode.length !== 24
      ) {
        ...
        if (nav) {
          ...
          this._queue("performance_summary", {
            ...
            domContentLoaded: Math.max(
              0,
              this.metrics.performance.domContentLoaded || 0,
            ),
    ...
    SublyzerSDK.prototype._setupErrors = function () {
                ...
                return origFetch
                  ...
                  .catch((err) => {
                    pushCrumb({
                      ...
                      ms: Date.now() - start,
                      message: String(
                        (err && err.message) || err || "fetch error",
                      ).slice(0, 300),
                    });
                    throw err;
                  });
      ...
      const _fetchJson = async (url) => {
        try {
          ...
          const len =
            Number(
              (r.headers && r.headers.get && r.headers.get("content-length")) ||
                "0",
            ) || 0;
      ...
      const _getSm = async (absJs) => {
        try {
          ...
          if (_smPending && _smPending.has(absJs))
            return await _smPending.get(absJs);
          const job = (async () => {
      ...
      const _resolveFrames = async (frames) => {
        try {
          ...
          for (const fr of frames) {
            ...
            const repoPath = _normSource(
              pos.source,
              sm.map && sm.map.sourceRoot,
            );
            if (!repoPath) {
              out.push(fr);
              continue;
        ...
        document.addEventListener(
          ...
          (e) => {
            try {
              const a =
                e.target &&
                (e.target.closest ? e.target.closest("a[href]") : null);
              if (!a) return;
        ...
        try {
          const push = (f) =>
            this._queue("vulnerability", {
              ...
              matchIndex:
                typeof f.matchIndex === "number" ? f.matchIndex : undefined,
              sources: f.sources,
                ...
                if (idx >= 0) {
                  ...
                  try {
                    if (abs && abs.startsWith(location.origin)) {
                      ...
                      if (/\.js($|\?)/.test(abs))
                        candidates.push(
                          abs.replace(/\.js(\?.*)?$/, ".js.map$1"),
                        );
                      if (
                        !(this && this.config && this.config.enableSourceMaps)
                      ) {
                        /* skip */
                      } else
                        for (const u of candidates) {
              ...
              for (const h of securityHeaders) {
                ...
                if (!val) {
                  push({
                    ...
                    severity:
                      h === "content-security-policy" ? "medium" : "low",
      ...
      document.addEventListener(
        ...
        (e) => {
          ...
          this.metrics.userActions =
            (Number(this.metrics.userActions) || 0) + 1;
      ...
      document.addEventListener("submit", (e) => {
        try {
          ...
          for (const el of inputs) {
            ...
            if (
              !nameVal &&
              /(name|full.?name|first.?name|last.?name)/i.test(nameAttr)
            ) {
              nameVal = val;
            }
        ...
        this.metrics.formSubmitCount =
          (Number(this.metrics.formSubmitCount) || 0) + 1;
        this._queue("user_action", {
          type: "form_submit",
      ...
      for (const ev of events) {
        try {
          if (ev.type === "error") {
            ...
            ev.type === "performance" ||
            ev.type === "performance_summary"
          ) {
            items.push({
              dataType: "performance",
              data: {
                loadTime:
                  ev.data?.loadTime || this.metrics?.performance?.loadTime || 0,
                domContentLoaded:
                  ev.data?.domContentLoaded ||
  ...
  const _scriptEl =
    document.currentScript || document.querySelector('script[src*="sublyzer"]');
  ...
  const _isSublyzerHost =
    typeof window !== "undefined" &&
    /(^|\.)sublyzer\.com$/i.test(window.location.hostname);
  ...
  const _dbg =
    typeof _g.debug === "boolean" ? _g.debug : _ds.debug === "true" || false;
