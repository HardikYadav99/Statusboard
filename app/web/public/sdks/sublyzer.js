...
(function () {
  const SublyzerSDK = (function () {
    ...
    SublyzerSDK.prototype._validate = async function () {
      try {
        const r = await fetch(
          this.config.apiUrl + "/data-collection/integration/" + this.config.integrationCode + "/validate"
        );
        if (!r.ok) return;
    ...
    SublyzerSDK.prototype._setupPerformance = function () {
      ...
      try {
        ...
        this._log("Performance monitoring error", e, "error");
      }
    };
    ...
    SublyzerSDK.prototype._setupErrors = function () {
      ...
      const parseStack = (stack) => {
        try {
          ...
          for (const line of lines) {
            ...
            if (m) {
              out.push({ function: "", file: normalizeFile(m[1]), line: Number(m[2] || 0), col: Number(m[3] || 0), raw: l });
              continue;
            }
            m = l.match(/^(.*?)@(.*?):(\d+):(\d+)\s*$/);
            if (m) {
      ...
      const makeFingerprint = (msg, frames) => {
        const top = (frames || [])
          .slice(0, 6)
          .map((f) => String((f && f.file) || "") + ":" + String((f && f.line) || 0) + ":" + String((f && f.col) || 0))
          .join("|");
      ...
      window.addEventListener("unhandledrejection", async (event) => {
        try {
          ...
          this._queue("error", {
            type: "unhandled_promise_rejection",
            message: (event && event.reason && event.reason.message) || "Unhandled promise rejection",
      ...
      const runScan = async () => {
        try {
          ...
          try {
            ...
            if (!hasCsp)
              push({
                type: "CSP Missing",
                severity: "medium",
                description: "Missing CSP meta tag",
              ...
              document.querySelectorAll("[" + attr + "]").forEach((el) => {
                ...
                if (code)
                  push({
                    type: "Inline Handler",
                    severity: "medium",
                    description: "Inline handler " + attr + " detected",
                    resource: location.href,
          ...
          const pats = [
            {
              ...
              severity: "critical",
              description: "eval/new Function detected",
            },
            {
              type: "XSS",
            ...
            },
            {
              ...
              re: /\.innerHTML\s*=\s*/g,
              severity: "medium",
              description: "innerHTML= detected",
            },
            {
              type: "Open Redirect",
            ...
            for (const n of nodes) {
              ...
              if (url && /^http:\/\//i.test(url)) {
                push({
                  ...
                  severity: "medium",
                  description: "Insecure HTTP resource",
                  resource: url,
          ...
          try {
            ...
            let resp = null;
            try {
              resp = await fetch(location.href, { method: "HEAD", cache: "no-store", credentials: "same-origin" });
            } catch (_) {}
            if (!resp) {
              try {
                resp = await fetch(location.href, { method: "GET", cache: "no-store", credentials: "same-origin" });
              } catch (_) {}
            }
              ...
              for (const h of securityHeaders) {
                ...
                if (!val) {
                  push({
                    ...
                    description: "Missing response header: " + h,
                    resource: location.href,
                    snippet: h,
    ...
    SublyzerSDK.prototype._setupUserTracking = function () {
      ...
      document.addEventListener("submit", (e) => {
        try {
          ...
          if (foundEmail) {
            try {
              this.trackEvent &&
                this.trackEvent("identify", { email: foundEmail, name: nameVal || undefined, userId: userIdVal || undefined });
            } catch (_) {}
          }
        } catch (_) {}
        this.metrics.userActions = (Number(this.metrics.userActions) || 0) + 1;
      ...
      if (!this._sessionActive) {
        ...
        this._queue("user_metric", { sessions: 1, ts: new Date().toISOString() });
      }
    };
    ...
    SublyzerSDK.prototype.trackEvent = function (eventType, data) {
      ...
      if (eventType === "cookie_analysis") {
        this._queue("cookie", { ...(data || {}), ts: new Date().toISOString() });
        return;
      }
      if (eventType === "download_tracking") {
        this._queue("download", { ...(data || {}), ts: new Date().toISOString() });
        return;
      }
    ...
    SublyzerSDK.prototype.trackConversionStep = function (step) {
      try {
        this.trackEvent && this.trackEvent("conversion_step", { step: String(step || "unknown") });
      } catch (_) {}
    };

    SublyzerSDK.prototype.trackAbExposure = function (experiment, variant) {
      try {
        this.trackEvent && this.trackEvent("ab_exposure", { experiment: String(experiment || "EXPERIMENT"), variant: String(variant || "A") });
      } catch (_) {}
    };

    SublyzerSDK.prototype.trackConversion = function (experiment, amount) {
      try {
        this.trackEvent && this.trackEvent("conversion", { experiment: experiment || undefined, amount: Number(amount || 0) });
      } catch (_) {}
    };
        ...
        try {
          if (ev.type === "error") {
            ...
            items.push({
              ...
              data: {
                loadTime:
                  (ev.data && ev.data.loadTime) || (this.metrics && this.metrics.performance && this.metrics.performance.loadTime) || 0,
                domContentLoaded:
                  (ev.data && ev.data.domContentLoaded) ||
                  (this.metrics && this.metrics.performance && this.metrics.performance.domContentLoaded) ||
                  0,
            ...
            if (et === "cookie_analysis") {
              ...
              items.push({
                ...
                data: { userId: this.userId, event: { eventType: "identify", payload } },
                source: "browser",
                userAgent: navigator.userAgent,
                sessionId: this.sessionId,
              });
              ...
              items.push({
                dataType: "custom_event",
                data: { userId: this.userId, event: { eventType: et, payload } },
                source: "browser",
                userAgent: navigator.userAgent,
                sessionId: this.sessionId,
              });
  ...
  // FIX: Port 5001 added for your local proxy route!
  const _apiDefault = _isLocal
    ? "http://localhost:5001/sublyzer"
    : _isSublyzerHost
    ? "/api/accelerate"
  ...
  try {
    sdk.trackEvent("hello_world", { message: "Sublyzer active" });
  } catch (_) {}
